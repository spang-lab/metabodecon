...
#' @title Get the signal free region in SU (strange units).
#' @description Converts the signal free region from ppm to SU (strange units). We use the name "strange unit", because I cannot see any sensible interpretation for the kind of transformation that is used to convert the signal free region from ppm to SU.
get_signal_free_region_in_su <- function(ppm_values, signal_free_region_in_ppm, scale_factor_x) {
    n <- length(ppm_values)
    ppm_max <- max(ppm_values)
    ppm_min <- min(ppm_values)
    ppm_range <- ppm_max - ppm_min
    ppm_step <- ppm_range / n
    sfr_left <- (n - (ppm_max - signal_free_region_in_ppm[1]) / ppm_step) / scale_factor_x
    sfr_right <- (n - (ppm_max - signal_free_region_in_ppm[2]) / ppm_step) / scale_factor_x
    sfr <- c(sfr_left, sfr_right)
    sfr
}
