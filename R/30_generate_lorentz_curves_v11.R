...
#' @noRd
generate_lorentz_curves_v11 <- function(data_path,
    ...
    for (i in seq_along(files)) {
        ...
        x <- deconvolution_v11(filepath, name, file_format, same_parameter, processing_value, number_iterations, range_water_signal_ppm, signal_free_region, smoothing_param, delta, scale_factor, current_filenumber = i, number_of_files = length(files))
...
#' @details Successor of [decovolution](R/MetaboDecon1D.R)
#' @noRd
deconvolution_v11 <- function(filepath = "urine_1/10",
                              name = NULL,
                              file_format = "bruker",
