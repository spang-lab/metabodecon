    ...
    if (!is.null(fq_width) && !is_equal(fq_width_calc, fq_width)) {
        if (force) {
            msg <- paste(
                ...
                "`force = TRUE` to ignore this error. Please note that by",
                "doing so, all downstream calculations involving frequencies",
                "might be wrong, so be sure to double check the results."
            )
            stop(sprintf(msg, round(fq_width_calc, 5), round(fq_width, 5)))
        } else if (!silent) {
            msg <- paste(
                "Calculated spectrum width in Hz (%s) does not match the",
                "provided value (%s). Continuing anyways, because `force`",
                "equals `TRUE`. Please note that all downstream calculations",
                "using frequencies might be wrong, so be sure to double check",
                "the results."
            )
...
#' @export
#'
#' @title Harmonize a corpus of spectra onto a shared chemical-shift grid
#'
#' @description
#' Pre-aligns every spectrum in `x` to a single shared chemical-shift
#' grid by integer-datapoint shifting. After the call, every spectrum's
#' `$cs` is bit-identical to the chosen target grid, so downstream code
#' (deconvolution, [metabodecon::clupa()], [metabodecon::snap_to_ref()],
#' feature-matrix builders) can index everyone by datapoint and treat
#' `cs` as a single shared variable.
#'
#' All input spectra must share the same point count and the same ppm
#' width (calibration offsets are allowed, frequency-domain resolution
#' differences are not). For each spectrum the integer offset from the
#' target grid is computed in datapoints, `$si` is rolled by that
#' many positions, the vacated edge is filled with `pad` (default `0`),
#' and `$cs` is replaced by the target grid.
#'
#' This kills the per-spectrum absolute-calibration offset between
#' acquisitions (typically a constant shift in ppm coming from
#' spectrometer reference setup) so that downstream CluPA only has to
#' correct the remaining sub-datapoint residual + the random
#' chemical-shift drift from sample composition.
#'
#' Sub-datapoint residual: at most ±0.5 datapoint per spectrum, which
#' is two orders of magnitude smaller than typical Lorentzian peak
#' widths (lambda ~ 1e-3 to 1e-2 ppm vs. spacing ~ 1e-4 ppm), so the
#' rounding error is irrelevant for any downstream fit.
#'
#' Edge handling: a spectrum that needs to shift right by `k`
#' datapoints loses `k` datapoints from one end and gains `k` `pad`
#' values on the other. For typical NMR spectra the edges are noise,
#' so zero-padding is equivalent to dropping the noise — no real
#' signal is harmed. For large absolute shifts (tens of datapoints)
#' you can verify the discarded region is noise by inspecting `$cs`
#' against the metabolite range of interest.
#'
#' @param x A `spectra` object (or list of `spectrum` objects).
#' @param target Either `"median"` (default) — anchor the target grid
#'   at the median first-ppm across `x` — or a numeric vector of
#'   length `length(x[[1]]$cs)` giving an explicit target grid.
#' @param pad Numeric scalar used to fill the vacated edge after the
#'   shift. Default `0`. Use `NA` if you want downstream code to
#'   detect the borrowed region explicitly.
#'
#' @return A `spectra` object with every spectrum's `$cs` replaced by
#'   the target grid and `$si` shifted accordingly. All other fields
#'   (`$meta`, `$lcpar`, `$sit`, ...) are preserved unchanged.
#'
...
harmonize_grid <- function(x, target = "median", pad = 0) {
    ...
    if (length(unique(ns_pts)) != 1L) stop(
        "harmonize_grid: all spectra must have the same number of points ",
        "(saw ", paste(unique(ns_pts), collapse=", "), ").", call.=FALSE
    )
    widths <- vapply(x, function(s) abs(s$cs[n] - s$cs[1]), numeric(1))
    if (diff(range(widths)) > 1e-6) stop(
        "harmonize_grid: all spectra must have the same ppm width ",
        "(range across corpus: ", signif(diff(range(widths)), 3), " ppm).",
        call.=FALSE
    )

    if (identical(target, "median")) {
        anchor <- stats::median(vapply(x, function(s) s$cs[1], numeric(1)))
        s1cs <- x[[1]]$cs
        target_cs <- s1cs - (s1cs[1] - anchor)
    } else {
        stopifnot(is.numeric(target), length(target) == n)
        target_cs <- as.numeric(target)
    }

    spacing <- abs(target_cs[2] - target_cs[1])
    for (i in seq_along(x)) {
        s <- x[[i]]
        dp <- as.integer(round((s$cs[1] - target_cs[1]) / spacing))
        if (dp != 0L) {
            new_si <- rep(pad, n)
            src <- seq_len(n) + dp
            ok <- src >= 1L & src <= n
            new_si[ok] <- s$si[src[ok]]
            s$si <- new_si
        }
        s$cs <- target_cs
        x[[i]] <- s
...
#' simulated using [metabodecon::simulate_spectrum()] and stored in Bruker format using
#' [metabodecon::save_spectrum()], an additional file called `simpar` is created, containing
...
read_one_r <- function(spldir,
    ...
    if (dtypp == 0) {
        ...
        msg <- sprintf(
            paste(
                "Reading '%s' as 64 bit floating point numbers, because",
                "processing parameter DTYPP equals '%s' and `force == TRUE`.",
                "This behaviour is untested, so please double check the",
                "returned values. For details see",
                "Bruker_NMR_Data_Formats.pdf."
            ),
            path_1r,
            dtypp
        ...
        msg <- sprintf(
            paste(
                "Processing parameter `DTYPP` has value '%s' but only '0' is",
                "supported. This indicates that the intensity values in file",
                "'%s' are stored doubles and not as integers. To read the",
                "file nonetheless, set `force = TRUE`, but note that this",
                "behaviour is completely untested, so please double check the",
                "returned values."
            ),
            dtypp,
            path_1r
