...
#' @param foc_fill Backghround color of the rectangle around the focus region.
...
#'     plot_spectrum(spec, foc_rgn = NULL)
#'
...
plot_spectrum <- function(decon,
                          ...
                          foc_rgn  = c(0.40, 0.35),
                          foc_unit = "fraction",
                          foc_fill = rgb(1, 0.5, 0, alpha = 0.05),
                          foc_col  = "orange",
                          ...
                          sub_lc_show  = TRUE,
                          sub_sup_show = TRUE,
                          # Settings for Main Figure, depending on Sub Figure
                          ysquash  = NULL,
                          trp_show = NULL,
                          lc_show  = NULL,
                          sup_show = NULL,
                          ...
                          sup_lty = 1,
                          # Connecting Lines
                          connect_line_col = foc_col) {

    dev_code <- quote({
        sim_01 <- metabodecon_file("sim/sim_01")
        decon <- generate_lorentz_curves(sim_01, sfr = c(3.42, 3.58), ws = 0, ask = FALSE, verbose = FALSE, delta = 0.1)
        args <- stub("plot_spectrum", decon = decon, foc_rgn = c(3.49, 3.45), foc_unit = "ppm")
    })
    ...
    draw_sub_fig <- !is.null(sub_fig) && !is.null(foc_rgn)
    ysquash <- if (draw_sub_fig) sub_fig[3] * 0.96 else 0.96
    trp_show <- if (draw_sub_fig) TRUE else FALSE
    lc_show <- if (draw_sub_fig) TRUE else FALSE
    sup_show <- if (draw_sub_fig) TRUE else FALSE

    # Draw full spectrum if sub_fig is NULL, else focussed region
    main <- plot_spec(decon,
        foc_rgn = foc_rgn, foc_unit = foc_unit, foc_only = foc_only,
        foc_fill = foc_fill, foc_col = foc_col,
        mar = mar, box_col = box_col, axis_col = axis_col, fill_col = fill_col,
        line_col = line_col,
        ysquash = ysquash, yscale10 = yscale10,
        trp_show = TRUE, trp_col = trp_col, trp_pch = trp_pch,
        lc_show = lc_show, lc_col = lc_col, lc_lty = lc_lty,
        sup_show = sup_show, sup_col = sup_col, sup_lty = sup_lty,
    )
    ...
    if (!is.null(sub_fig) && !is.null(foc_rgn)) {
        ...
        sub <- plot_spec(decon,
            foc_rgn, foc_unit = foc_unit, foc_only = TRUE,
            mar = sub_mar, box_col = sub_box_col, axis_col = sub_axis_col,
            fill_col = sub_fill_col, line_col = sub_line_col,
            yscale10 = yscale10,
            fig = c(xndc, yndc), add = TRUE
        )
    }
    ...
    if (!is.null(sub_fig) && !is.null(foc_rgn)) {
        browser()
        line1_x0 <- grconvertX(main$foc$ndc$xleft, "ndc", "user")
        line2_x0 <- grconvertX(main$foc$ndc$xright, "ndc", "user")
        line1_x1 <- grconvertX(sub$plt$ndc$xlim[1], "ndc", "user")
        line2_x1 <- grconvertX(sub$plt$ndc$xlim[2], "ndc", "user")
        y0 <- grconvertY(main$foc$ndc$ytop, "ndc", "user")
        y1 <- grconvertY(sub$plt$ndc$ylim[1], "ndc", "user")
        opar <- par(xpd = TRUE)
...
plot_spec <- function(decon,
    ...
    axs <- ps_draw_axis(dat, axis_col, box_col)
...
ps_init_plot_region <- function(dat = ps_get_dat()) {
    ...
    ndc <- list(
        xlim = grconvertX(usr$xlim, to = "ndc"),
        ylim = grconvertY(usr$ylim, to = "ndc")
    )
    do.call(plot, usr)
    named(usr, ndc)
}
...
ps_draw_axis <- function(dat, axis_col, box_col) {
    xtks <- seq(dat$xlim[1], dat$xlim[2], length.out = 5)
    ytks <- seq(dat$ylim[1], dat$ylim[1], length.out = 5)
    xlab <- "Chemical Shift [ppm]"
    ylab <- "Signal Intensity [au]"
    axis(
        side = 1, at = xtks, labels = round(xtks, 2),
        col = NA, col.ticks = axis_col, col.axis = axis_col
    )
    axis(
        side = 2, at = ytks, labels = round(ytks, 2),
...
ps_draw_lorentz_curves <- function(dat = ps_get_dat(),
    ...
    for (i in seq_along(dat$A)) {
        ...
        if (lc_show) {
            lines(x = x, y = Y[, i], col = lc_col, lty = lc_lty, type = "l")
        }
    }
    if (sup_show) {
