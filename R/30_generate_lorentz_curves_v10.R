...
#' @export
#' @title Generate Lorentz Curves
#' @description Starts the actual deconvolution of your spectra and fits a Lorentz curve for each detected signal.
...
#' @param file_format Either `"bruker"` or `"jcampdx"`.
#' @param make_rds Store results as file `"spectrum_data.rds"` in folder `data_path`? If many spectra are evaluated and therefore computing time increases, this should be set to TRUE.
#' @examples \donttest{
#' xds_path <- download_example_datasets()
#' data_path <- file.path(xds_path, "bruker/urine")
#' spectrum_data <- generate_lorentz_curves(data_path = data_path, file_format = "bruker")
#' }
...
generate_lorentz_curves_v10 <- function(data_path, file_format = "bruker", make_rds = FALSE) {
    spectrum_data <- MetaboDecon1D(data_path, file_format = file_format)
    if (make_rds) saveRDS(object = spectrum_data, file = file.path(data_path, "spectrum_data.rds"))
    spectrum_data
}

test_generate_lorentz_curves_v10 <- function(cache = TRUE, overwrite = FALSE) {
