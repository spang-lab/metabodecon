...
#' @examples \dontrun{after_speaq_mat <- speaq_align(feat, maxShift)}
...
#' @examples \dontrun{aligned_res <- combine_peaks(after_speaq_mat, range, lower_bound, spectrum_data, data_path)}
#' @details As result two .csv files will be generated. One that contain all columns for each data point in the original spectrum one column (aligned_res_long.csv) and one where all columns that contain only zeros (aligned_res_short.csv) are omitted. Both files will be stored in the directory specified by data_path.
#' @export
...
#' @title get_ppm_range
#' @description Get ppm range of your spectra
#' @param spectrum_data Output of [generate_lorentz_curves()]
#' @examples \donttest{ppm_range <- get_ppm_range(spectrum_data = spectrum_data)}
