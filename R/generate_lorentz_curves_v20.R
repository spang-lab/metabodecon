...
remove_negative_signals_v20 <- function(spec) {
    if (is.null(spec$Y$nows)) stop("Water signal not removed yet. Please call `remove_water_signal_v20()` first.")
...
find_peaks_v20 <- function(spec) {
    d <- spec$d <- calc_second_derivative_v20(y = spec$Y$smooth, bwc = bwc)
    a <- abs(d)
    m <- length(d)
