...
read_spectrum <- function(path, type = "bruker", sf = c(1e3, 1e6), expno = 10, procno = 10) {
    ...
    sfy <- sf[2]
    switch(type,
        "bruker" = load_bruker_spectrum_v2(path, sfx, sfy, expno, procno),
        "jcampdx" = load_jcampdx_spectrum_v2(path, sfx, sfy)
    )
}
...
#' str(spectrum_data, 1)
#' }
#' @details Never published and superseded by `load_jcampdx_spectrum_v2()`.
#' @noRd
...
#' str(spectrum_data, 1)
#' }
#' @details Never published and superseded by `load_bruker_spectrum_v2()`.
#' @noRd
...
#' spectrum_data <- load_bruker_spectrum_v2(path) # takes 3s on machine r31
#' str(spectrum_data, 1)
#' }
#' @noRd
