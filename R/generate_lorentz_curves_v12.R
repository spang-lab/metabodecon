...
# Deprecated helpers of deconvolution_v12 #####

refine_lorentz_curves_example <- function() {
    # Get result and intermediate states from old function run
    func_result <- get_func_result(rds = "deconvolution_urine1_spF_ni10_cf1_nf2.rds")
    func_state <- func_result$rv$debuglist

    # Use the function state before the parameters optimization as inputs
    inputs <- within(list(), {
        spectrum_x <- func_state$params_init$spectrum_x
        spectrum_y <- func_state$params_init$spectrum_y
        filtered_peaks <- func_state$params_init$filtered_peaks
        filtered_left_position <- func_state$params_init$filtered_left_position
        filtered_right_position <- func_state$params_init$filtered_right_position
        A <- func_state$params_init$A
        lambda <- func_state$params_init$lambda
        w <- func_state$params_init$w
        number_iterations <- func_state$params_init$number_iterations
    })

    # Call the function
    lcr <- do.call(refine_lorentz_curves, inputs)

    str(func_state, 2)

    # Assert that the result equals the function state after the parameters optimization
    vcomp(func_state$params_approx$w_new, lcr$w_new)
    vcomp(func_state$params_approx$lambda_new, lcr$lambda_new)
    vcomp(func_state$params_approx$A_new, lcr$A_new)
}

refine_lorentz_curves <- function(spectrum_x,
                                  spectrum_y,
                                  filtered_peaks,
