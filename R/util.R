...
download_example_datasets <- function(dst_dir = NULL,
                                      extract = TRUE,
                                      cache_persistent = NULL,
                                      ask = interactive()) {
    cached_zip <- cache_example_datasets(persistent = cache_persistent, ask = ask)
    if (!is.null(dst_dir)) {
        file.copy(cached_zip, dst_dir)
    }
    if (extract) {
        dst_zip <- file.path(dst_dir, basename(cached_zip))
        extract_example_datasets(dst_zip)
    }
}
...
#' cache_example_datasets(persistent = FALSE)
#' @export
cache_example_datasets <- function(persistent = FALSE, overwrite = FALSE) {
...
datadir <- function(file = "", warn = TRUE, persistent = NULL) {
    datadir_persistent <- tools::R_user_dir("metabodecon", "data")
    datadir <- datadir_temp <- file.path(tempdir(), "data")
