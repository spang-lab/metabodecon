...
![Metabodecon Workflow](metabodecon_workflow.svg)
...
- ᶜ The change is backwards compatible if you ignore global state, such as files
  written to disk or output printed to STDOUT. However, if a script expects such
  side effects, it will fail. Therefore we set this to "semi".
- ᵈ The faster implementation also fixes an indexing bug, which in most cases
  shouldn't have any effect, but in some rare cases it might cause one peak to
  be missed.
