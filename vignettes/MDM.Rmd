...
`clupa()` calls an internal `find_ref()` to pick the spectrum whose
...
ref <- metabodecon:::find_ref(decons)
...
The equivalent one-shot call uses `fit_mdm()` with `model="ranger"`
...
internal engine `metabodecon:::fit_mdm_internal()`, which accepts
