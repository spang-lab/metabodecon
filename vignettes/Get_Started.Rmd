...
- For more information on the Sim and Blood datasets, refer to the [Datasets] article.
- For a description of the problems that Metabodecon tries to solve, refer to the [Purpose] article.
- For an introduction into NMR spectroscopy, refer to the [NMR-Basics] article.
...
deconvs <- metabodecon::generate_lorentz_curves(
    ...
    delta = 0.2          # Configure threshold for peak filtering
)
```
...
Deconvolution results for the first two spectra of the Sim dataset. The raw SI (black), smoothed SI (blue), and superposition of Lorentz curves (red) are closely aligned, indicating that `smopts` and `delta` were chosen well and that the deconvolution was successful.
</figcaption>
</figure>
