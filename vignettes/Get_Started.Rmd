...
metabodecon::plot_spectrum(deconvoluted_spectra[[1]], foc_rgn = c(0.5, 0.2))
...
To visualize the data before and after the alignment, you can use `plot_spectra()`. To do the alignment, you can use `align_spectra()`:

```R
metabodecon::plot_spectra(deconvoluted_spectra)
aligned_spectra <- metabodecon::align_spectra(deconvoluted_spectra)
