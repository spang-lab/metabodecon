...
```R
simdir <- metabodecon::metabodecon_file("bruker/sim")
decons <- metabodecon::generate_lorentz_curves(
    data_path = simdir,  # Path to directory containing spectra
...
metabodecon::plot_spectrum(decons[[1]], foc_rgn = c(0.5, 0.2))
...
The following code snippets show how the above steps can be done using the 'metabodecon' package:
...
```R
ppm_range <- metabodecon::get_ppm_range(spectrum_data = decons, show = TRUE)
ppm_range_str <- paste(round(ppm_range, 3), collapse = " - ")
cat(sprintf("PPM Range: %s\n", ppm_range_str))
## PPM Range: 3.455 - 3.547
```
...
To generate a matrix of features based on the spectrum data, you can use `gen_feat_mat()`. This function will return a list containing

- `data_matrix`: A data.frame where each row corresponds to one spectrum and each column to one data point, i.e. for 16 input spectra with 1309 data points we get dimension 16 x 1309.
...
- `w` / `lambda` / `lambda`: A list of vectors where each vector contains the "position / area / width paramater" of the peaks in the corresponding spectrum.

```R
feat <- metabodecon::gen_feat_mat(decons)
str(feat, 1)
## List of 5
##  $ data_matrix: num [1:16, 1:1309] 0.01351 0.02418 ...
##  $ peakList   :List of 16
##  $ w          :List of 16
##  $ A          :List of 16
##  $ lambda     :List of 16
...
Now that we have one single matrix containing all spectra, we can align them using `speaq_align()`. The resulting matrix will have one row for each spectrum and one column for each chemical shift.

```R
after_speaq_mat <- metabodecon::speaq_align(
  feat = feat,
  maxShift = 50,
  spectrum_data = decons
)
str(after_speaq_mat, 2)
```

Parameter `maxShift` of `speaq_align()` give the maximum number of points along the x axis a value can be moved by the 'speaq' package. A value of 50 may be used as start value for plasma spectra. However, depending on your spectra and the used digital resolution this value may be be adapted.

## 4. Combine peaks

```R
aligned_res <- metabodecon::combine_peaks(
