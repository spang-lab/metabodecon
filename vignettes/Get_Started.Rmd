...
The goal of metabodecon is to make deconvolution and alignment of 1D NMR spectra as easy as possible. The deconvolution part uses the codebase from [MetaboDecon1D](https://www.doi.org/10.3390/metabo11070452) and implements the algorithm from ([Koh et al, 2009](#koh-et-al-2009)). The alignment of the deconvoluted spectra is done using functions from the [speaq](https://CRAN.R-project.org/package=speaq) package ([Beirnaert et al., 2018](#beirnaert-et-al-2018)) that have been adapted for metabodecon.
...
library(metabodecon)
xdspath <- download_example_datasets(persistent = FALSE)
