test_that("smooth_signals works", {
    y <- c(1, 4, 2, 4, 3)
    spec <- list(y_pos = y)
...
test_that("runtime(smooth_signals2) < runtime(smooth_signals)", {
    r1 <- n <- 0
    while (r1 == 0) {
        spec <- list(y_pos = rnorm(2^10 * 2^(n <- n + 1)))
        r1 <- system.time(smooth_signals(spec, k = 5, reps = 10, verbose = FALSE))[["elapsed"]]
    }
    r2 <- system.time(smooth_signals2(spec$y_pos, k = 5, reps = 10))[["elapsed"]]
