...
test_that("deconvolute works", {
    ...
    # Call Function
    args <- named(x, nfit, smopts, delta, sfr, wsr, rtyp, rmwsv, force)
    runf <- function(...) do.call(deconvolute_x_check_args, set(args, ...))
    # styler: off
    ...
    expect_error(runf(rmwsv  = NA ), "rmwsv must be a integer.1., not .*")
