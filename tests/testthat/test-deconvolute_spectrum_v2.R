# CONTINUE HERE
# ── Failed tests ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
# Error (test-deconvolute_spectrum_v2.R:25:5): 1. deconvolute_spectrum_v2(format = bruker, sameparam = FALSE)
# Error in `deconvolute_spectrum_v2(path = "urine_1", type = "bruker", same_parameter = FALSE, 
#     spectroscopy_value = 10, processing_value = 10, number_iterations = 1, 
#     range_water_signal_ppm = 0.1527692, signal_free_region = c(11.44494, 
#         -1.8828), smoothing_param = c(2, 5), delta = 6.4, scale_factor = c(1000, 
...
#                 number_iterations = 1,
#                 range_water_signal_ppm = 0.1527692,
#                 signal_free_region = c(109.09458303373, 21.8529143006947), # When using the same parameters as for the last run, the signal_free_region is not interpreted as "signal free region in ppm" but as "signal free region in xxx" where "xxx" is the unit used by `deconvolute_spectrum_v2()$signal_free_region`. From what I could see by skimming over the code, this unit is something like a "scaled rank". I.e. if signal_free_region_in_ppm[1] is 11.44 and 11.44 is the 109094th smallest value in the spectrum, then `deconvolute_spectrum_v2()$signal_free_region[0]` will be `109094/scale_factor[1]`, which is 109.094 if scale_factor[1] is 1000.
#                 smoothing_param = c(2, 5),
#                 delta = 6.4,
#                 scale_factor = c(1000, 1000000),
