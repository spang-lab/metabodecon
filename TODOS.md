...
- Go through all "check" - Todos and discuss which one of them really needs a fix
- Discuss proposed [Version Matrix](#version-matrix) and decide for a name of `F4` (see [Feature Matrix](#feature-matrix))
- Discuss publication date of MetaboDecon1D and cumulative Dissertation

# Feature Matrix

| Feature                                   | F1  | F2  | F3  | F4  | BWC   | Todo       |
| ----------------------------------------- | --- | --- | --- | --- | ----- | ---------- |
| Doesn't write to disk by default          |     | x   | x   | x   | no^2  | CRAN-8     |
| Doesn't change wd or global opts          |     | x   | x   | x   | no^2  | CRAN-9     |
...
| Doesn't show License                      |     |     | x   | x   | no^2  | REFACTOR-2 |
| Prints timestamps                         |     |     | x   | x   | no^2  | REFACTOR-2 |
...
- F4: `metabodecon::deconvolute_spectra` or `metabodecon::generate_lorentz_curves_2`
...
- ^2: The change is backwards compatible if you ignore global state, such as files written to disk. However, if a script expects such files to exist after the function call, it will fail. Therefore we set this to "no".

# Version Matrix

| Package       | Version     | F1  | F2  | F3  | F4  |
| ------------- | ----------- | --- | --- | --- | --- |
| MetaboDecon1D | == 0.2.2    | e   |     |     |     |
| metabodecon   | >= 1.1, < 2 |     | ed  | e   | i   |
| metabodecon   | >= 2.0, < 3 |     | i   | ed  | e   |
